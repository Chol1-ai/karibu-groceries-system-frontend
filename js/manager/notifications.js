...
(function() {
    ...
    const refreshUnreadCount = async () => {
        try {
            ...
            // Ignore badge refresh errors to avoid noisy UX.
        }
    };
    ...
    const openPanel = async () => {
        try {
            ...
            if (rows.length > 0) {
                await window.KGL.API.post('/notifications/read-all', {});
            }
            setBadges(0);
        } catch (error) {
    ...
    const list = () => [];
    window.KGL.Notifications = { openPanel, list, refreshUnreadCount };
    window.ManagerNotifications = { list };
})();
